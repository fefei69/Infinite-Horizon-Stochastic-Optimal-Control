...
def value_iteration(mdp:MDP, num_iter):
    """
    V, pi = value_iteration(mdp, num_iter)
    """
    # terminal and nontermina states
    term_state = np.isfinite(mdp.Y)
    ntrm_state = ~term_state
    # initialize the policy and value
    pi = np.zeros((num_iter+1,mdp.nX),dtype='int')
    V = np.zeros((num_iter+1,mdp.nX))
    V[:,term_state] = mdp.Y[term_state] # set the value of the terminal states
    # import pdb; pdb.set_trace() 
    
    # value iteration    
    for k in range(num_iter):
        Q = mdp.L[ntrm_state,:] + mdp.gamma * np.sum(mdp.P[ntrm_state,:,:] * V[k,None,None,:], axis=2) # num_ntrm x nA
        pi[k+1,ntrm_state] = np.argmin(Q, axis=1)        
        V[k+1,ntrm_state] = np.min(Q,axis=1)
    return V, pi



def policy_iteration(mdp:MDP, num_iter):
    """
...
def random_episode(env):
    ...
    for t in range(100):
        ...
        ob, re, done, _, prob = env.step(a)
        env.render()
        if done or t == 99: break
